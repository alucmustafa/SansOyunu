...
let sec= 100;
                         //slot reel rotation speed(runs per second)
let stopReelFlag =[];
                           //slot reel stop flag
let reelCounts = [];
                           //which image to position
let slotFrameheight;
                           //frame size
let slotReelsHeight;
                           //overall reel (image) size
let slotReelitemHeight;
                           //size of one reel (image)
let slotReelStartheight;
                           //iinitial image value


let isDark= document.querySelector("#isDark");

isDark.addEventListener("click",function(){
  if(document.body.contains(".Dmode")){
    document.body.classList.add("Wmode")
  }
  else{
